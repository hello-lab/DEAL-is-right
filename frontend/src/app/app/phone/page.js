...
export default function PhonePage() {
		...
		fetch('http://127.0.0.1:8000/analytics')
			...
			.then(data => {
				if (Array.isArray(data.analytics)) {
					const filtered = data.analytics.filter(item => item.type?.toLowerCase() === 'phone').map(item => ({
						menuName: item.type,
	...
	return (
		<div style={{ padding: 24 }}>
			<h2 style={{ marginBottom: 24, fontWeight: 700, fontSize: '2em', color: '#1976d2' }}>Phones</h2>
			<div className="tiles" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '24px' }}>
				...
				{phones.length === 0 && (
					<div style={{ color: '#888', marginTop: 16 }}>No phones found.</div>
				)}
			</div>
		</div>
	);
